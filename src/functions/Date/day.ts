import { DaysWeekPtBR } from "./Day/types";
...
function yesterday(date?: Date): Date {
  ...
  yesterday.setDate((date?.getDate() ?? new Date().getDate()) - 1);
...
function tomorrow(date?: Date): Date {
  ...
  tomorrow.setDate((date?.getDate() ?? new Date().getDate()) + 1);
